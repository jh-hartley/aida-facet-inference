...
async def len_safe_get_embedding(text: str) -> tuple[list[list[float]], list[int]]:
    chunk_embeddings = []
    chunk_lengths = []
    
    for chunk in _chunked_tokens(text, config.EMBEDDING_DEFAULT_DIMENSIONS):
        chunk_text = tiktoken.get_encoding(config.OPENAI_EMBEDDING_MODEL).decode(chunk)
...
async def len_safe_get_averaged_embedding(text: str) -> list[float]:
    ...
    averaged_embedding = np.average(chunk_embeddings, axis=0, weights=chunk_lengths)
    normalised_embedding = averaged_embedding / np.linalg.norm(averaged_embedding)
