...
async def main():
    parser = argparse.ArgumentParser(description="Run a facet inference experiment")
    parser.add_argument(
        "--description",
        type=str,
    ...
    with SessionLocal() as session:
        ...
        if args.product:
            logger.info(f"Running experiment for single product: {args.product}")
            experiment_key = await orchestrator.run_experiment(limit=1)
        else:
            experiment_key = await orchestrator.run_experiment(limit=args.limit)
